...
code init		( -- ) \ Initialize kvm.g.members that are moved out from jeforth.js which is thus kept pure.
						...
						for (var i=0; i < a.length; i++){
							...
							if (ta == tb) {
								...
								if (ta == "object") {  // 怎麼比較 obj? v2.05 之後用 memberCount()
									if (kvm.g.memberCount(a[i]) != kvm.g.memberCount(b[i])) return false;
				...
				// return {flag, key}
				kvm.g.isMember = function (item, thing){
				...
				// 缺點是 kvm.g.setTimeout.registered() 會大量堆積，需 delete(kvm.g.setTimeout.registered()[id.toString()]) 既然還得記住
				// timeoutId 使得 kvm.g.setTimeout() 的好處大打折扣。 查看： js> kvm.g.setTimeout.registered() (see)
				// setInterval 比較不會大量堆積，最好還是要適時 delete。查看：js> kvm.g.setInterval.registered() (see)
				kvm.g.setInterval = (function(){
					var registered={};
					f = function(a,b){
				...
				kvm.g.debugInner = function (entry, resuming) {
...
// debugger; // [ ]
                if (!kvm.g.isSameArray(stackwas,stack)) {
...
: constant 		( n <name> -- ) \ Create a 'constnat', Don't use " in <name>.
				BL word (create) <js> 
				last().type = "constant";
				var s = 'var f;f=function(){push(kvm.g["' 
						+ last().name 
						+ '"])}';
				last().xt = eval(s);
				kvm.g[last().name] = pop();
...
: to 			( n <value> -- ) \ Assign n to <value>.
				...
				compiling if ( word ) 
					<js> var s='var f;f=function(){/* to */ kvm.g["'+pop().name+'"]=pop()}';push(eval(s))</js> ( f ) ,
				else ( n word )
					js: kvm.g[pop().name]=pop()
				then ; immediate
...
: sleep 		( mS -- ) \ Suspend to idle, resume after mS. Can be 'stopSleeping'.
				...
				<js>
					function resume() { 
						...
						delete(kvm.g.setTimeout.registered()[me.timeoutId.toString()]);
					...
					if (me.timeoutId) {
						...
						me.timeoutId = kvm.g.setTimeout(resume,delay);
					}
				</js> ;
...
: nap			( mS -- ) \ Suspend to idle, resume after mS. Multiple nap is allowed.
				<js>
					...
					var timeoutId = kvm.g.setTimeout(resume,delay);
					function resume() { 
						delete(kvm.g.setTimeout.registered()[timeoutId.toString()]);
						tib = tibwas; ntib = ntibwas;
						outer(ipwas); // resume to the below ending 'ret' and then go through the TIB.
					}
				</js> ;
				/// nap 不用 kvm.g.setTimeout 故不能中止，也不會堆積在 kvm.g.setTimeout.registered() 裡。
...
code memberCount ( obj -- count ) \ Get hash table's length or an object's member count.
				push(kvm.g.memberCount(pop()));
				end-code

code isSameArray ( a1 a2 -- T|F ) \ Compare two arrays.
				push(kvm.g.isSameArray(pop(), pop()));
				end-code
...
code (see)      ( thing -- ) \ See into the given word, object, array, ... anything.
                ...
                if (!(w instanceof Word)) {
                    kvm.g.see(w);  // none forth word objects. 意外的好處是不必有 "unkown word" 這種無聊的錯誤訊息。
                }else{
                    for(var i in w){
